...
export function NewHome() {
  ...
  const askHost = async () => {
    ...
    const dir = selection().directory ?? projects()[0]?.worktree
    if (!dir) {
      showToast({
      ...
      >
        <div class="mx-auto grid min-h-full w-full max-w-[1080px] grid-rows-[auto_minmax(0,1fr)_auto] gap-4 px-3 lg:grid-cols-[280px_minmax(0,720px)] lg:grid-rows-1 lg:gap-8 lg:px-6">
          {(platform.platform === "ios" || platform.platform === "android") && (
            <p class="block text-center mt-2 text-12-regular text-text-dimmed lg:col-span-2">
          ...
          <Show when={pushMode()} keyed>
            {(mode) => (
              <div class="mx-auto mt-6 w-full max-w-xl rounded-2xl border border-border-weak-base bg-[linear-gradient(180deg,var(--surface-raised-base),color-mix(in_srgb,var(--surface-raised-base)_84%,transparent))] p-4 shadow-lg shadow-black/5 lg:col-span-2">
