...
export function TerminalPanelV2(props: { stacked?: boolean; mobileFull?: boolean } = {}) {
  ...
  const mobileFullHeight = createMemo(() => {
    ...
    return `${Math.max(120, store.viewTop + store.view - store.rootTop)}px`
  })
  ...
  onMount(() => {
    ...
    const sync = () => {
      ...
      setStore("rootTop", root?.getBoundingClientRect().top ?? 0)
    }
    ...
    if (port) {
      makeEventListener(port, "resize", sync)
      makeEventListener(port, "scroll", sync)
    }
  })
  ...
  function ExtraKeysBar() {
    ...
    return (
      <div class="flex shrink-0 gap-0.5 overflow-x-auto px-2 py-1 bg-background-stronger border-b border-border-weaker-base [&>button]:shrink-0">
        <button
        ...
        >
          ...
          >
            <div class="flex flex-col h-full">
              ...
              <Show when={!isDesktop() && platform.platform === "mobile"}>
                <ExtraKeysBar />
              </Show>
